...
class ChannelVideosTvFragment: VerticalGridSupportFragment(),
    ...
    private fun setupRecyclerViewOnScrollListener(){
        ...
        view?.findViewById<VerticalGridView>(androidx.leanback.R.id.browse_grid)?.addOnScrollListener(object :OnScrollListener(){
            override fun onScrolled(recyclerView: RecyclerView, dx: Int, dy: Int) {
                ...
                last10ChildView.forEach { tenVisibleItemPositions.add(it?.let { it1 ->
    ...
    private fun loadMore(){
        ...
        channelHandle?.let { channel ->
            ChikiFetcher().fetchVideosOfaChannel(channel, loadStartNumber, sortBy = sort).observe(this
            ) { vids ->
                var videos = vids
    ...
    private fun setupFilter(){
        ...
        val dialog = AlertDialog.Builder(requireActivity()).setPositiveButton(getString(R.string.apply),DialogInterface.OnClickListener { dialogInterface, i ->

            selectedSortPosition = spinner?.selectedItemPosition ?: 0
            showRaws = dialogView.findViewById<CheckBox>(R.id.tv_filter_japanese_raw_check_box).isChecked
            loadVideos()
        ...
        spinner?.adapter = ArrayAdapter(requireActivity(),android.R.layout.simple_spinner_dropdown_item, sortArray)
    ...
    private fun showNoEnglishVideosTextView(){
        ...
        noEnglishVideosTextView.setTextColor(ContextCompat.getColor(requireActivity(),R.color.orange))
        noEnglishVideosTextView.layoutParams = FrameLayout.LayoutParams(FrameLayout.LayoutParams.WRAP_CONTENT,FrameLayout.LayoutParams.WRAP_CONTENT).apply { gravity = Gravity.CENTER }
        noEnglishVideosTextView.textSize = 24f
        noEnglishVideosTextView.typeface = ResourcesCompat.getFont(requireActivity(),R.font.mochiypoppone)

        view?.findViewById<FrameLayout>(androidx.leanback.R.id.browse_grid_dock)?.addView(noEnglishVideosTextView)
    }

    private fun hideNoEnglishVideosTextView(){
        view?.findViewById<FrameLayout>(androidx.leanback.R.id.browse_grid_dock)?.removeView(noEnglishVideosTextView)
    }
